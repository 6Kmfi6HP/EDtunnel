...
let userID = '6a869ac9-7a12-40f7-b766-82a76dd87d42';

const proxyIPs = ['36.95.152.58:443'];
...
function getConfig(userIDs, hostName) {
  ...
  `;
	...
	const configOutput = userIDArray.map((userID) => {
		...
		const protocolSec = atob(pt) + '://' + userID + atob(at) + hostName + commonUrlPart1;
		return `
      <div class="container config-item">
        <h3>Port 443</h3>
        <div class="code-container">
          <pre><code>${protocolMain}</code></pre>
          <button class="btn copy-btn text-light" onclick='copyToClipboard("${protocolMain}")'><i class="fas fa-copy text-light"></i> Copy</button>
        </div>
        
        <h3>Port 80</h3>
        <div class="code-container">
          <pre><code>${protocolSec}</code></pre>
          <button class="btn copy-btn text-light" onclick='copyToClipboard("${protocolSec}")'><i class="fas fa-copy text-light"></i> Copy</button>
        </div>
      </div>
    `;
	}).join('');
...
const hostnames = [
	't.me/Tingkeh',            // Secoo - A Chinese luxury e-commerce platform
];
