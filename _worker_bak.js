			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					case '/connect': // for test connect to cf socket
						const [hostname, port] = [proxyIP, '443'];
						const connect_info = `Connecting to ${hostname}:${port}...`
						const connect_error = `Failed to connect to ${hostname}:${port}`
						try {
...
/**
 * Generates a VLESS configuration for a given set of userIDs and hostName.
 * @param {string} userIDs - A comma-separated string of userIDs.
